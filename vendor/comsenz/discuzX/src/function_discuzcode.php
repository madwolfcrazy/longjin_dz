...
function dhtmlspecialchars($string, $flags = null,$charset='gbk') {
	if(is_array($string)) {
...
function tpl_hide_credits_hidden($creditsrequire) {
    return '<div class="">Creadits Hidden block</div>';
}

function tpl_hide_credits($creditsrequire, $param2){
    return '<div class="">Creadits Hidden credits</div>';
}
...
function tpl_hide_reply() {
    return '<div class="showhide"><h4>{lang post_hide}</h4>\\1</div>';
}
function tpl_hide_reply_hidden() {
    return '<div class="locked"><!--{if $_G[uid]}-->{$_G[username]}<!--{else}-->{lang guest}<!--{/if}-->{lang post_hide_reply_hidden}</div>';
}
...
function tpl_codedisp($code) {
    return '<div class="tpl-code">TPL Code</div>';
}
...
function discuzcode($message, $smileyoff = false, $bbcodeoff = false, $htmlon = 0, $allowsmilies = 1, $allowbbcode = 1, $allowimgcode = 1, $allowhtml = 0, $jammer = 0, $parsetype = '0', $authorid = '0', $allowmediacode = '0', $pid = 0, $lazyload = 0, $pdateline = 0, $first = 0) {
	...
	if($parsetype != 1 && !$bbcodeoff && $allowbbcode && (strpos($message, '[/code]') || strpos($message, '[/CODE]')) !== FALSE) {
		$message = preg_replace_callback("/\s?\[code\](.+?)\[\/code\]\s?/is", __NAMESPACE__.'\discuzcode_callback_codedisp_1', $message);
	}
	...
	if($_G['setting']['allowattachurl'] && strpos($msglower, 'attach://') !== FALSE) {
		$message = preg_replace_callback("/attach:\/\/(\d+)\.?(\w*)/i", __NAMESPACE__.'\discuzcode_callback_parseattachurl_12', $message);
	}

	if($allowbbcode) {
		if(strpos($msglower, 'ed2k://') !== FALSE) {
			$message = preg_replace_callback("/ed2k:\/\/(.+?)\//", __NAMESPACE__.'\discuzcode_callback_parseed2k_1', $message);
		}
	}

	if(!$bbcodeoff && $allowbbcode) {
		if(strpos($msglower, '[/url]') !== FALSE) {
			$message = preg_replace_callback("/\[url(=((https?|ftp|gopher|news|telnet|rtsp|mms|callto|bctp|thunder|qqdl|synacast){1}:\/\/|www\.|mailto:)?([^\r\n\[\"']+?))?\](.+?)\[\/url\]/is", __NAMESPACE__.'\discuzcode_callback_parseurl_152', $message);
		}
		if(strpos($msglower, '[/email]') !== FALSE) {
			$message = preg_replace_callback("/\[email(=([a-z0-9\-_.+]+)@([a-z0-9\-_]+[.][a-z0-9\-_.]+))?\](.+?)\[\/email\]/is", __NAMESPACE__.'\discuzcode_callback_parseemail_14', $message);
		}

		$nest = 0;
		while(strpos($msglower, '[table') !== FALSE && strpos($msglower, '[/table]') !== FALSE){
			$message = preg_replace_callback("/\[table(?:=(\d{1,4}%?)(?:,([\(\)%,#\w ]+))?)?\]\s*(.+?)\s*\[\/table\]/is", __NAMESPACE__.'\discuzcode_callback_parsetable_123', $message);
			if(++$nest > 4) break;
		}
		...
		if(!defined('IN_MOBILE')) {
			if(strpos($msglower, '[/media]') !== FALSE) {
				$message = preg_replace_callback("/\[media=([\w,]+)\]\s*([^\[\<\r\n]+?)\s*\[\/media\]/is", $allowmediacode ? __NAMESPACE__.'\discuzcode_callback_parsemedia_12' : __NAMESPACE__.'\discuzcode_callback_bbcodeurl_2', $message);
			}
			if(strpos($msglower, '[/audio]') !== FALSE) {
				$message = preg_replace_callback("/\[audio(=1)*\]\s*([^\[\<\r\n]+?)\s*\[\/audio\]/is", $allowmediacode ? __NAMESPACE__.'\discuzcode_callback_parseaudio_2' : __NAMESPACE__.'\discuzcode_callback_bbcodeurl_2', $message);
			}
			if(strpos($msglower, '[/flash]') !== FALSE) {
				$message = preg_replace_callback("/\[flash(=(\d+),(\d+))?\]\s*([^\[\<\r\n]+?)\s*\[\/flash\]/is", $allowmediacode ? __NAMESPACE__.'\discuzcode_callback_parseflash_234' : __NAMESPACE__.'\discuzcode_callback_bbcodeurl_4', $message);
			}
		} else {
			if(strpos($msglower, '[/media]') !== FALSE) {
	...
	if(!$bbcodeoff) {
		if($parsetype != 1 && strpos($msglower, '[swf]') !== FALSE) {
			$message = preg_replace_callback("/\[swf\]\s*([^\[\<\r\n]+?)\s*\[\/swf\]/is", __NAMESPACE__.'\discuzcode_callback_bbcodeurl_1', $message);
		}
	...
	if($jammer) {
		$message = preg_replace_callback("/\r\n|\n|\r/", __NAMESPACE__.'\discuzcode_callback_jammer', $message);
	}
	if($first) {
